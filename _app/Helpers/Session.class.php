...
class Session {
    ...
    private function setSession() {
        $_SESSION['useronline'] = [
            ...
            "online_starview" => date('Y-m-d H:i:s'),
    ...
    private function sessionUpdate() {
        ...
        $_SESSION['useronline']['online_url'] = filter_input(INPUT_SERVER, 'REQUEST_URI', FILTER_VALIDATE_URL);
    }

    /*
     * ****************************************
     * **** USUÁRIO, VISITAS, ATUALIZAÇÕES ****
     * ****************************************
     */
    ...
    //Atualiza table com dados de navegadores
    private function BrowserUpdate() {
        $readAgent = new Read;
        $readAgent->ExeRead('ws_siteviews_anget', "WHERE agent_name = :agent", "agent={$this->Browser}");
        if (!$readAgent->getResult()):
            $ArrAgent = ['agent_name' => $this->Browser, 'agent_views' => 1];
            $createAgent = new Create;
            $createAgent->ExeCreate('ws_siteviews_anget', $ArrAgent);
        else:
            $ArrAgent = ['agent_views' => $readAgent->getResult()[0]['agent_views'] + 1];
            $updateAgent = new Update;
            $updateAgent->ExeUpdate('ws_siteviews_anget', $ArrAgent, "WHERE agent_name = :name", "name={$this->Browser}");
        endif;
    }
